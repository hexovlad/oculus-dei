...
class DatabaseConnector:
    def __init__(self, database_url: str,
        ...
        self.connection_string = self._create_connection_string()  # Creating the connection string
    ...
    def _create_connection_string(self) -> str or None:
        ...
        if self._DATABASE_USER is not None and self._DATABASE_PASSWORD is not None and self._DATABASE_AUTH_NAME is None:
...
class DatabaseActions:
    ...
    def data_get(self, collection: str, query: dict) -> dict or None:
        ...
        try:
            ...
            print(f'Error while looking for the following in the database.\n QUERY:\n{query}\n ', str(e))
    ...
    def data_modify_one(self, collection: str, filter_query: dict, updated_data: dict) -> str or None:
        """Modifies the data on a given key"""
        try:
            db_collection = self.db_connector.db[collection]
            result = db_collection.update_one(filter_query, updated_data)  # Updating the entry based on the filter
    ...
    def data_modify_many(self, collection: str, filter_query: dict, updated_data: dict) -> str or None:
        """Modifies the data on a given key"""
        try:
            db_collection = self.db_connector.db[collection]
            result = db_collection.update_many(filter_query, updated_data)  # Updating the entry based on the filter
    ...
    def data_delete_one(self, collection: str, filter_query: dict) -> str or None:
        ...
        try:
            db_collection = self.db_connector.db[collection]
            result = db_collection.delete_one(filter_query)  # Updating the entry based on the filter
    ...
    def data_delete_many(self, collection: str, filter_query: dict) -> str or None:
        ...
        try:
            db_collection = self.db_connector.db[collection]
            result = db_collection.delete_many(filter_query)  # Updating the entry based on the filter
